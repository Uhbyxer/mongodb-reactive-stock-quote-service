...
public class StockQuoteClient {
	...
	public Flux<Quote> getQuoteStream() {
		String uri = "http://".concat(host).concat(":").concat(port);

		log.debug("url is {}", uri);

		return WebClient.builder().baseUrl(uri).build().get().uri(path).accept(MediaType.APPLICATION_STREAM_JSON)
				.retrieve().bodyToFlux(Quote.class);
	}
}
